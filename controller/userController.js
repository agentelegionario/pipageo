...
db.Pa.find({}, function(error, pa) {
      ...
      if(error) {
          ...
          callback({error: 'Não foi possivel retornar um usuário'});
...
new db.Pa({
  ...
  'geolocaPAGET': geolocaPAGET,
   created_at: new Date
        ...
        if(error) {
            callback({error: 'Usuário não pode ser inserido'});
        } else {
            callback(pa);
        }
    });
...
exports.update = function(id, placa, geolocaPA, geolocaPAGET, callback){
    ...
    db.Pa.findById(id, function(error, pa) {
        ...
        if(error){
            ...
            callback({error: 'Não foi possível atulizar usuário'});
        } else {
            callback(pa);    
            }
        });
    ...
    db.Pa.findById(id, function(error, pa) {
        ...
        if(!error) {
            pa.remove(function(error) {
                if(!error) {
                    callback({error: 'Usuário ' +id+ ' excluido com sucesso' });
                } 
            });
        } else if(error == null) {
            
            callback({error: 'Usuário' +id+ ' não encontrado' });
        }
    });
}; 
//----------------------------------------------------------------------